  ...
  int NUM_THREADS= BLOCKS*THREADS_PER_BLOCK;
    #pragma omp parallel num_threads(NUM_THREADS)
    {
      ...
      if(shuffled_workgroup < stress_params[9]) { // 9 = testingWG
        ...
        test_locations[x_0].store(1, memory_order_release); // data
        test_locations[y_0].store(1, memory_order_release);  // flag
        
        int r0 = test_locations[y_1].load(memory_order_acquire);  // flag
        int r1 = test_locations[x_1].load(memory_order_acquire);  // data
        read_results[id_1 * 2 + 1].store(r1); // r1 = data
        read_results[id_1 * 2].store(r0); // r0 = flag
      }
...
void run(Result *count_local){
    ...
    for (int i=0; i<testLocSize; i++){
      testLocations[i].store(0, memory_order_relaxed);
    }
    for (int i=0; i<12; i++){
