// array of data
// array of data_dummy
// data[first half] = 0 = CPU
// data[second half] = 0 = GPU
// for (i=0; i< 100; i++)
    // data[tid] += data_dummy[i]
...
__global__ void init_first_half_data_dummy(atomic<int>* flag, int* data, int *data_dummy, int* result0/*flag*/, int*result1/*data*/) {
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    if (tid < (THREADS/2)){
        for(int j=0; j<THREADS; j++)
            data[tid] += data_dummy[j];
    }
}
...
void run(Result *count_local){
    ...
    if (data_in_unified_memory) {
        ...
        SAFE(cudaMallocManaged(&data_dummy, sizeof(int) * THREADS));
    } else {
    ...
    // init data_dummy to 0 first
    SAFE(cudaMemset(data_dummy, 0, THREADS));
    // first half GPU
    init_first_half_data_dummy<<<1, 512>>>(flag, data, data_dummy, result0, result1);   // note only 50% threads
    SAFE(cudaDeviceSynchronize());
    // second half CPU
    for (int i = 512; i < THREADS; ++i) {
        for(int j=0; j<THREADS; j++)
            data[i] += data_dummy[j];
    }
